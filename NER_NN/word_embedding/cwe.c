...
//Unicode range of Kannada characters
#define MIN_KANNADA 0x0C80
#define MAX_KANNADA 0x0CFF
...
struct vocab_word {
  ...
  /*
   * character[i]   : Unicode(the i-th character in the word) - MIN_KANNADA
...
int AddWordToVocab(char *word, int is_non_comp) {
  ...
  for (i = 0; i < len; i++)
    if (wstr[i] < MIN_KANNADA || wstr[i] > MAX_KANNADA) {
  ...
  for (i = 0; i < len; i++) {
    if (cwe_type == 1 || cwe_type == 3 || cwe_type == 5) {
      //character
      vocab[vocab_size - 1].character[i] = wstr[i] - MIN_KANNADA;
      ...
      if (len == 1)
        ...
        pos = 3;
      vocab[vocab_size - 1].character[i] = (wstr[i] - MIN_KANNADA) * 4 + pos;
    }
  }
...
void InitNet() {
  ...
  if (cwe_type) {
    ...
    if (cwe_type == 5) {
      a = posix_memalign((void **)&embed_count, 128, (MAX_KANNADA - MIN_KANNADA + 1) * sizeof(int));
      if (embed_count == NULL) {printf("Memory allocation failed\n"); exit(1);}
      a = posix_memalign((void **)&last_emb_cnt, 128, (MAX_KANNADA - MIN_KANNADA + 1) * sizeof(int));
      if (last_emb_cnt == NULL) {printf("Memory allocation failed\n"); exit(1);}
      for (a = 0; a < (MAX_KANNADA - MIN_KANNADA + 1); a++)
        embed_count[a] = 0;
    }
  }
  ...
  if (strlen(char_init)) {
    ...
    if (b != dim) {
      ...
      while (a--) {
        ...
        b = buf[0];
        if (b < MIN_KANNADA || b > MAX_KANNADA) continue;
        b -= MIN_KANNADA;
        t1 = b, t2 = b + 1;
...
void TrainModel() {
  ...
  if (strlen(output_char)) {
    ...
    if (cwe_type == 5) {
      tot = 0;
      for (a = 0; a <= MAX_KANNADA - MIN_KANNADA; a++)
        tot += embed_count[a];
    } else
      tot = character_size;
    fprintf(fo, "%lld\t%lld\n", tot, dim);
    for (a = 0; a < character_size; a++) {
      if (cwe_type == 1) {
        ...
        if (cwe_type == 2)
          ch[0] = MIN_KANNADA + a / 4;
        else
          ch[0] = MIN_KANNADA + a / 4 / multi_emb;
        ch[1] = 0;
        if (cwe_type == 2)
          c = a % 4;
        else
...
int main(int argc, char **argv) {
  ...
  if (cwe_type == 1)
    character_size = (MAX_KANNADA - MIN_KANNADA + 1);
  else if (cwe_type == 2)
    character_size = (MAX_KANNADA - MIN_KANNADA + 1) * 4;
  else if (cwe_type == 3 || cwe_type == 5)
    character_size = (MAX_KANNADA - MIN_KANNADA + 1) * multi_emb;
  else if (cwe_type == 4)
    character_size = (MAX_KANNADA - MIN_KANNADA + 1) * 4 * multi_emb;
  printf("CWE-Type: %d\n", cwe_type);
